...
fn connect_to_server(twamp_server_ip: String, twamp_server_port: u16) -> Result<TcpStream, String> {
...
fn request_tw_session(tcp_stream: &mut TcpStream, local_ip: &Ipv4Addr) -> Result<u16, String> {
...
fn stop_session(tcp_stream: &mut TcpStream) -> Result<(), String> {
    // Check if tcp stream is connected.
    if !tcp_stream.peer_addr().is_ok() {
        return Err("TCP Stream is not connected ... ".to_string());
    }
